...
import { rabbitmqRaitingUsecase } from '../../../dependencies';
import CustomError  from '../../../../application/errors/CustomError';

export class ConsumerRaiting {
    private channel: any;
    private queue: string;

    constructor() {
        this.queue = process.env.RABBITMQ_QUEUE || 'raiting-supplier-queue';
    }
        ...
        try {
            ...
            this.channel.consume(this.queue, async (msg: any) => {
                ...
                try {
                    ...
                    const { userUuid, polaridad  } = message;

                    console.log('userUuid', userUuid);
                    console.log('polarity', polaridad );
                    ...
                    const raitingResponse = await rabbitmqRaitingUsecase.execute(userUuid, polaridad );

                    console.log('paymentResponse', raitingResponse);
    
                    if (!raitingResponse || raitingResponse.status !== 200) {
                        console.error('No se encontraron datos para el usuario con UUID:', userUuid);
                        this.channel.nack(msg, false, false); // Rechazar mensaje
                        return;
                    }
    
                    const raitingData = raitingResponse;
                    console.log('raitingData', raitingData.data);
                    // Añadir los datos al mensaje
                    message.userData = {
                        fullname: raitingData.data,
                    };
